...
func GetColumnValue(data []map[string]interface{}, colname string) (interface{}, error) {
	if len(data) == 0 {
		return nil, fmt.Errorf("empty data slice")
	}

	firstMap := data[0]
	value, ok := firstMap[colname]
	if !ok {
		return nil, &ColumnNotFoundError{Column: colname}
	}

	return value, nil
}

func RunQueryAndReturnColumnByName(db *sql.DB, qs string, colname string) string {
	rows, err := ExecuteQueryWithDatabase(qs, db)
	if err != nil {
		fmt.Println(err)
	}
	//row := rows[0]
	val, err := GetColumnValue(rows, colname)
